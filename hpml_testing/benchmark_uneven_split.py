...
def setup_model(args, strategy, dtype):
    ...
    if strategy == "ring" and dist.is_initialized():
        ...
        shard_mode = os.environ.get("SHARD_MODE", "even").lower()

        # -----------------------------
        # Per-rank "speed" from MPS (same idea as test_ring_prefill.py)
        # -----------------------------
        # If you don't set CUDA_MPS_ACTIVE_THREAD_PERCENTAGE, this defaults to 100 for all ranks.
