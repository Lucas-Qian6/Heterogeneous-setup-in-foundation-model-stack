...
SIZES=${SIZES:-"2048 4096 8192"}
MPS_PCTS=${MPS_PCTS:-"100 80 60 40 20"}
DTYPE=${DTYPE:-float16}
NUM_ITERS=${NUM_ITERS:-20}
WARMUP_ITERS=${WARMUP_ITERS:-5}
BG_STREAMS=${BG_STREAMS:-0}
OUT_CSV=${OUT_CSV:-"hpml_testing/results/matmul_mps_sweep_$(date +%Y%m%d_%H%M%S).csv"}
...
for pct in $MPS_PCTS; do
  ...
  for n in $SIZES; do
    echo "=== MPS=${pct}% size=${n} ==="
    # Capture a single line of benchmark output matching the requested size.
    line=$(python "$SCRIPT_DIR/benchmark_single_gpu_matmul.py" \
      --sizes "$n" \
      --dtype "$DTYPE" \
      --num-iters "$NUM_ITERS" \
